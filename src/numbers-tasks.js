...
function roundToPowerOfTen(num, pow) {
  const numbersArr = num.toString().split('').map(Number);
  let answer = 0;
  if (pow === 0) {
    answer = num;
  } else {
    for (let i = 1; i <= pow; i += 1) {
      const digitIndex = numbersArr.length - i;
      if (numbersArr[digitIndex] >= 5) {
        numbersArr[digitIndex] = 0;
        numbersArr[digitIndex - 1] += 1;
      } else {
        numbersArr[digitIndex] = 0;
      }
    }
  }
  answer = Number(numbersArr.join(''));
  return answer;
}
